...
/* local globals */
BAN_LIST_ELEM *ban_list = NULL;

/* local functions */
void load_banned(void);
int isbanned(char *hostname);
void _write_one_node(FILE *fp, BAN_LIST_ELEM *node);
...
void load_banned(void)
{
  ...
  while (fscanf(fl, " %s %s %d %s ", ban_type, site_name, &date, name) == 4) {
    CREATE(next_node, BAN_LIST_ELEM, 1);
...
ACMD(do_ban)
{
  ...
  *buf = '\0';
  ...
  CREATE(ban_node, BAN_LIST_ELEM, 1);
