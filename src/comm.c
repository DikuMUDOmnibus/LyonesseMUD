...
/* externs */
extern BAN_LIST_ELEM	*ban_list;
extern TIME_INFO_DATA	time_info;		/* In db.c */
...
int main(int argc, char **argv)
{
	...
	if (pos < argc)
	{
		if (!isdigit(*argv[pos]))
		{
			printf("Usage: %s [-c] [-m] [-r] [-s] [-d pathname] [port #]\n", argv[0]);
			exit(1);
		}
	...
	while (!circle_shutdown)
	{
		...
		for (d = descriptor_list; d; d = next_d)
		{
			...
			if (d->character)
			{
				...
				if (STATE(d) == CON_PLAYING && GET_WAS_IN(d->character) != NULL)
				{
...
void heartbeat(int pulse)
{
	...
	/* check for wilderness room to be removed */
	if (!(pulse % PULSE_WILDREM))
		wild_remove_update();
	...
	else if (STATE(d) == CON_PLAYING && !IS_NPC(d->character))
	{
		...
		*sbuf   = '\0';
		...
		if (PLR_FLAGGED(d->character, PLR_GHOST))
			count += sprintf(prompt + count, "%s%s[Ghost]%s ", CCBLD(d->character, C_SPR),
		...
		{
			percent = percentage(GET_HIT(d->character), GET_MAX_HIT(d->character));

			if (percent >= 70)
				sprintf(sbuf, "%d", GET_HIT(d->character));
			else if (percent >= 35)
				sprintf(sbuf, "%s%s%d%s", CCBLD(d->character, C_SPR), CCYEL(d->character, C_SPR),
				GET_HIT(d->character), CCNRM(d->character, C_SPR));
			else 
				sprintf(sbuf, "%s%s%d%s", CCBLD(d->character, C_SPR), CCRED(d->character, C_SPR),
				GET_HIT(d->character), CCNRM(d->character, C_SPR));
			
			sprintf(prompt + strlen(prompt), "Hp:[%s] ", sbuf);
		}
		
		if (PRF_FLAGGED(d->character, PRF_DISPMANA))
		{
			percent = percentage(GET_MANA(d->character), GET_MAX_MANA(d->character));
			
			if (percent >= 70)
				sprintf(sbuf, "%d", GET_MANA(d->character));
			else if (percent >= 35)
				sprintf(sbuf, "%s%s%d%s", CCBLD(d->character, C_SPR), CCYEL(d->character, C_SPR),
			...
			sprintf(prompt + strlen(prompt), "Ma:[%s] ", sbuf);
		}

		if (PRF_FLAGGED(d->character, PRF_DISPMOVE))
		{
			percent = percentage(GET_MOVE(d->character), GET_MAX_MOVE(d->character));

			if (percent >= 70)
				sprintf(sbuf, "%d", GET_MOVE(d->character));
			else if (percent >= 35)
				sprintf(sbuf, "%s%s%d%s", CCBLD(d->character, C_SPR), CCYEL(d->character, C_SPR),
			...
			sprintf(prompt + strlen(prompt), "Mv:[%s] ", sbuf);
		}
...
char *parse_color(const char *txt, DESCRIPTOR_DATA *t)
{
	char *new_txt;
	char *toret;
	register int i, j = 0;
	
	i = count_chars(txt, '&'); /* count how many control-chars there are in the string */
	
	new_txt = malloc(i * 5 + strlen(txt) + 1);

	/*
	 * no ansi-escape code is larger
...
void close_socket(DESCRIPTOR_DATA *d)
{
  ...
  if (d->character) {
    ...
    if (STATE(d) == CON_PLAYING || STATE(d) == CON_DISCONNECT) {
      ...
      save_char(link_challenged, NULL);
...
void send_to_wildsect(WILD_DATA *wd, const char *messg)
{
	...
	for (i = descriptor_list; i; i = i->next)
	{
		...
		if (!AWAKE(i->character) || !IN_WILD(i->character))
			continue;
		if (wd != get_wd(i->character->in_room->coord))
			continue;
		SEND_TO_Q(messg, i);
	}
}
...
void perform_act(const char *orig, CHAR_DATA *ch, OBJ_DATA *obj,
		const void *vict_obj, const CHAR_DATA *to)
{
	const char *i = NULL;
	...
	for (;;)
	{
		if (*orig == '$')
		{
			...
			{
			case 'n':
				i = PERS(ch, (CHAR_DATA *)to);
				break;
			case 'N':
				CHECK_NULL(vict_obj, PERS((CHAR_DATA *) vict_obj, (CHAR_DATA *)to));
				break;
			case 'm':
				i = HMHR(ch);
				break;
	 ...
	 OBJ_DATA *obj, const void *vict_obj, int type)
{
	...
	if (type == TO_VICT)
	{
		if ((to = (const CHAR_DATA *) vict_obj) != NULL && SENDOK(to))
